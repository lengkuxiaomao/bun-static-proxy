...
- **零配置** — 默认读取 `./dist` 目录，也可通过 `DIST_DIR` 自定义路径
- **API 代理** — 内置反向代理，前端直接调后端接口，无需处理跨域
- **端口自适应** — 启动时自动检测端口占用，冲突时自动切换
- **超时保护** — 代理请求 85s 超时自动断开，返回 504，避免资源泄漏
...
```
.
├── dist/            # 前端构建产物（默认目录，可在 .env 中通过 DIST_DIR 自定义）
│   ├── index.html
│   ├── assets/
│   └── ...
├── server.ts        # 服务入口
...
> [!TIP]
> **静态资源目录可灵活配置。** 服务默认从 `./dist` 目录读取静态资源，也可以在 `.env` 中通过 `DIST_DIR` 参数指定任意相对路径或绝对路径。
...
将你的前端打包产物放到默认的 `./dist` 目录下，或者在 `.env` 中配置 `DIST_DIR` 指向对应的构建产物路径：
...
| `DIST_DIR`     | `./dist`                  | 静态资源目录路径（支持相对路径或绝对路径） |
